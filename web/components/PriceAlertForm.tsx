...
export function PriceAlertForm({
  ...
  return (
    <div
      className={`rounded-xl border border-slate-100 bg-slate-50/60 ${compact ? "p-3" : "p-4"} ${className}`}
    >
      <p className={`leading-snug text-slate-600 ${compact ? "text-xs" : "text-sm"}`}>
        <Bell
      ...
      <form
        className={`mt-3 flex flex-col gap-2 ${compact ? "" : "sm:flex-row sm:items-start"}`}
